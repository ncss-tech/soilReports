...
if (cache.samples & file.exists('cached-samples.Rda')) {
  ...
  n_rasters <- length(unique(sampling.res$raster.samples$variable))
...
"Total Area", "Polygons")
    ...
    res2 <- aggregate(samples$pID,
                      ...
                      FUN = function(x) {
                        c(length(x) / n_rasters, sum(!x %in% musub$pID))
                      })
...
```{r, echo=FALSE, warn=FALSE, eval=do.continuous}
...
suppressWarnings({names(mu.check)[-1] <- abbreviateNames(mu.check)})

# fix names for printing
suppressWarnings({names(polygons.to.check)[1] <- mu.col})
```


```{r, echo=FALSE, eval=do.continuous, warn = FALSE}
...
```{r interactive-map, echo=FALSE, eval=exists("interactive.map") && isTRUE(interactive.map), include=FALSE}
