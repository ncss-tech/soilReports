...
params:
   ...
   genhz_rules: !r as.character(file.path(system.file(package = "soilReports"), "reports/region11/lab_summary_by_taxonname/genhz_rules/Generic_rules.R"))
---
...
# fig.height
ssp_nh <- length(lh_s[lh_s %in% names(lh)]) / 4 * 2.5
dsp_nh <- length(lh_d[lh_d %in% names(lh)]) / 4
...
```{r site data}
kable(s[, c("pedon_id", "taxonname", "tax_subgroup", "part_size_class", "pedon_type", "describer")], cation = "Site Data")
```
