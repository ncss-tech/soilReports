...
output: 
  html_document: 
    ...
    toc_float:
      ...
      smooth_scroll: no
params:
  projectname: !r as.character("EVAL - MLRA 95 - Hortonville silt loam, 20 to 30 percent slopes;EVAL - MLRA 95 - Hortonville-Boyer-Zurich comples, 20 to 35 percent slopes;EVAL - MLRA 95 - Hortonville-Boyer-Zurich complex, 12 to 20 percent slopes, eroded;EVAL - MLRA 95 - Hortonville-Boyer-Zurich complex, 2 to 6 percent slopes;EVAL - MLRA 95 - Hortonville-Boyer-Zurich complex, 6 to 12 percent slopes, eroded")
---
...
cosm <- within(cosm, {
  ...
  compname2 = reorder(compname, comppct_r2, max)
  comp_id   = factor(paste(compname, "\n", dmuiid ,"\n" , comppct_r, "%", "\n", drainagecl))
  comp_id   = factor(comp_id, levels = unique(comp_id[order(compname, - dmuiid)]))
  })
...
idx <- with(s, !is.na(pmkind_grp) | !is.na(landform))
n_compname     <- ceiling(length(unique(paste(s$compname[idx], s$drainagecl[idx]))) / 3) + 0.2
h_cplots_pm  <- n_compname * 
                   ceiling(0.3 * max(c(
                     length(unique(s$pmkind_grp)),
                     length(unique(s$landform))
                     )))

idx <- with(s, !is.na(landform) | !is.na(geompos))
n_compname     <- ceiling(length(unique(paste(s$compname[idx], s$drainagecl[idx]))) / 3) + 0.2
h_cplots_lf  <- n_compname * 
                   ceiling(0.3 * max(c(
                     length(unique(s$landform)),
                     length(unique(s$geompos))
                     )))

idx <- with(s, !is.na(hillslopeprof) | ! is.na(geompos))
n_compname     <- ceiling(length(unique(paste(s$compname[idx], s$drainagecl[idx]))) / 3) + 0.2
h_cplots_2d  <- n_compname * 
                   ceiling(0.5 * max(c(
                     length(unique(s$hillslopeprof)),
                     length(unique(s$geompos))
                     )))

idx <- with(s, !is.na(hillslopeprof) | ! is.na(slopeshape))
n_compname     <- ceiling(length(unique(paste(s$compname[idx], s$drainagecl[idx]))) / 3) + 0.2
h_cplots_ss  <- n_compname * 
                   ceiling(0.5 * max(c(
                     length(unique(s$hillslopeprof)),
                     length(unique(s$slopeshape))
                     )))
...
datatable(with(s, { data.frame(dmudesc_short, compname, drainagecl, slope_r, pmgroupname, pmkind_grp, geompos, landscape, landform, slopeshape, hillslopeprof)}),
          options = list(pageLength = 10)
          )
```

```{r sm_pmkind, fig.height= h_cplots_pm, fig.ext = "png", dev = "png_dplots"}
# landform vs 3-D morphometry
s <- within(s, {
  dmu_type = ifelse(!grepl("MLRA", s$dmudesc), "nonmlra_dmu", "mlra_dmu")
  comp_sort = factor(paste0(compname, "\n", drainagecl))
  comp_sort = factor(comp_sort, levels = unique(comp_sort[order(- as.numeric(compname2), - dmuiid)]))
  })


. <- subset(s, ! is.na(pmkind_grp) | ! is.na(landform)) 
.$comp_sort <- droplevels(.$comp_sort)
ggplot(., aes(x = landform, y = pmkind_grp, fill = dmu_type)) + 
  geom_tile(alpha = 0.5) + 
  facet_wrap(~ comp_sort, ncol = 3) + 
  theme(legend.position="top", aspect.ratio = 1, axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  ggtitle("Parent Material Kind vs Landform by Component")
```

```{r sm_3d, fig.height = h_cplots_lf, fig.ext = "png", dev = "png_dplots"}
# landform vs 3-D morphometry
. <- subset(s, ! is.na(landform) | ! is.na(geompos)) 
.$comp_sort <- droplevels(.$comp_sort)
ggplot(., aes(x = geompos, y = landform, fill = dmu_type)) + 
  geom_tile(alpha = 0.5) + 
  facet_wrap(~ comp_sort, ncol = 3) + 
  theme(legend.position="top", aspect.ratio = 1, axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
...
```{r sm_2d, fig.height = h_cplots_2d, fig.ext = "png", dev = "png_dplots"}
# 2-D vs 3-D morphometry
. <- subset(s, !is.na(hillslopeprof) | ! is.na(geompos)) 
.$comp_sort <- droplevels(.$comp_sort)
ggplot(., aes(x = geompos, y = hillslopeprof, fill = dmu_type)) + 
  geom_tile(alpha = 0.5) + 
  facet_wrap(~ comp_sort, ncol = 3) + 
  theme(legend.position="top", aspect.ratio = 1, axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
...
```{r sm_ss, fig.height = h_cplots_ss, fig.ext = "png", dev = "png_dplots"}
# 2-D vs slope shape
. <- subset(s, !is.na(hillslopeprof) | ! is.na(slopeshape))
.$comp_sort <- droplevels(.$comp_sort)
ggplot(., aes(x = slopeshape, y = hillslopeprof, fill = dmu_type)) + 
  geom_tile(alpha = 0.5) + 
  facet_wrap(~ comp_sort, ncol = 3) + 
...
h3 <- merge(vars_slice, site(comp)[c("dmuiid", "comp_id", "coiid", "compname2", "majcompflag", "comppct_r", "localphase", "taxgrtgroup", "taxpartsize", "hydgrp")], by = "comp_id", all.x = TRUE)
...
gg_list <- lapply(vars, function(x) {
  h_sub = h5[c("dmuiid", "comp_id", "coiid", "compname2", "majcompflag", "comppct_r", "localphase", "taxgrtgroup", "taxpartsize", "hzname", "dep", "hzdept_r", "hzdepb_r", "hzdep_m",
