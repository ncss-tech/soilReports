...
n_depthplots  <- ceiling(n_coiid / 4) * 4
n_cosoilmoistiid <- length(unique(cosm$coiid))
n_cosoilmoist <- ceiling(n_cosoilmoistiid / 2) * 4
...
```{r horizon data, fig.height=n_profile}
# profile plot
...
h <- merge(vars_slice, site(comp)[c("dmuiid", "comp_id", "coiid")], by = "coiid", all.x = TRUE)

h2 <- by(h, paste(h$coiid, h$hzname), function(x) data.frame(
  coiid = x$coiid[1],
  hzname = x$hzname[1],
...
gg_list <- lapply(vars, function(x) {
  h_sub = h[c("comp_id", "coiid", "hzname", "dep", "hzdept_r", "hzdepb_r", "hzdep_m",
