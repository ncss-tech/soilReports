...
comp$comp_id <- factor(comp$comp_id, levels = unique(sort(comp$comp_id, decreasing = TRUE)))
comp$comp_id2 <- with(site(comp), paste(dmuiid, "\n", compname))
comp$comp_id2 <- factor(comp$comp_id, levels = unique(sort(comp$comp_id, decreasing = TRUE)))
...
n_cosoilmoist_h <- ceiling(length(unique(cosm$coiid)) / 4) * 4
# n_cosoilmoist_w <- length(unique(cosm$dmuiid)) * 6 
profile_idx <- pIndex(comp, 10)
n_profile <- ceiling(length(unique(comp$coiid)) / 10) * 4
...
```{r soil moisture, fig.height=n_cosoilmoist_h, fig.width=10}
# component soil moisture month
...
for (i in unique(profile_idx)){
  plot(comp[which(profile_idx == i)], name = 'hzname', label = 'comp_id2', id.style = "top")
  }
...
h <- merge(vars_slice, site(comp)[c("dmuiid", "comp_id", "coiid", "compname", "comppct_r")], by = "coiid", all.x = TRUE)

vars <- c("coiid", "hzname")
h2 <- by(h, apply(h[vars], 1, paste, collapse ="_"), function(x) data.frame(
  x[1, vars],
...
h2 <- do.call("rbind", h2)
h <- merge(h, h2, by = c("coiid", "hzname"), all.x = TRUE)
...
gg_list <- lapply(vars, function(x) {
  h_sub = h[c("dmuiid", "comp_id", "coiid", "compname", "comppct_r", "hzname", "dep", "hzdept_r", "hzdepb_r", "hzdep_m",
                 paste0(x, c("_l", "_r", "_h", "_min2")))]
  names(h_sub) = gsub(paste0(x, "_"), "", names(h_sub))
  
  h_sub$var = x
  # idx = apply(h_sub, 1, function(x) any(is.na(x["r"])))
  # h_sub = h_sub[!idx, ]
