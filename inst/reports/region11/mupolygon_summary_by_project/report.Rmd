...
output: 
  html_document: 
    ...
    toc_float:
      ...
      smooth_scroll: no
params:
   geodatabase: !r as.character("RTSD_R11-IND_FY17.gdb")
   project_data_file_path: !r as.character("M:/geodata/project_data/")
   ssoffice: !r as.character("R11-IND")
---
# Report Description
...
region_folder <- paste0(dir, "R11/")

# cache files
sample_cache <- paste0(office_folder, "cache/", unique(project$uprojectid), "_", samplePercent, ".Rdata")
gdb_cache <- gsub(".gdb", "", paste0(office_folder, geodatabase, "_cache.RData"))
shp_cache <- paste0(office_folder, "cache/", unique(project$uprojectid)[1], ".RData")
```
...
if (!file.exists(sample_cache)){
  ...
  if (
    ...
    sum(file.size(list.files(paste0(office_folder, geodatabase), full.names = TRUE))) / 1e+9 > 1.5 & 
    # QGIS is installed
    ...
    ogr_extract(office_folder, geodatabase, 
                paste0(office_folder, "cache/", unique(project$uprojectid)[1], ".shp"), project$mukey)
  ...
  if (
    ...
    } else if (file.exists(gdb_cache)) load(file = gdb_cache)
  ...
  mupolygon_sp <- by(mupolygons, mupolygons$idx, function(x) {
