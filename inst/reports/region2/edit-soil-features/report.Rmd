...
if (!is.null(params$INCLUDE_ADDITIONAL)) {
  if (!params$INCLUDE_ADDITIONAL)
...
s2 <- fetchSDA_spatial(f2$lmapunitiid, add.fields = c("mapunit.musym", "mapunit.muname"))
```
...
Total Acres: `r round(sum(f2$compacres, na.rm = TRUE))`
...
knitr::kable(common_components[order(common_components$Spatial_Acres, decreasing = TRUE), ],
             ...
             col.names = c("Component Name", "# Components", "Spatial Acres"))
```
...
```{r}
f2$mss <- getMineralSoilSurfaceDepth(f2)$hzdepb_r
...
```{r truncateSPC}
f0040 <- trunc(f2, 0, round(40 * 2.54))
f1040 <- trunc(f2, round(10 * 2.54), round(40 * 2.54))
f0010 <- trunc(f2, 0, round(10 * 2.54))
f0060 <- trunc(f2, 0, round(60 * 2.54))
```

# PERMEABILITY [0 to 10 inches]
```{r permeability_0to10, warning=FALSE}
permclasses <- c(
  Impermeable = 0,
  `Very Slow` = 0.0015,
  Slow = 0.06,
...
f0010$permeability <- findInterval(f0010$ksat_l, permclasses)
f0010 <- mutate_profile(f0010, min_permeability = as.integer(min(permeability, na.rm = TRUE)))
f0010$min_permeability <- names(permclasses)[f0010$min_permeability]
f0010.permeability <- table(f0010$min_permeability, useNA = "ifany")
f0010.permeability.acres <- aggregate(f0010$compacres, by = list(min_permeability = f0010$min_permeability), function(x) round(sum(x, na.rm = TRUE)))
f0010.permeability.acres$n <- f0010.permeability[f0010.permeability.acres$min_permeability]
knitr::kable(f0010.permeability.acres[order(f0010.permeability.acres$x, decreasing = TRUE),],
             col.names = c("Min. Permeability [0 to 10 inches]", "Component Acres", "# Components"),
             row.names = FALSE)
```

# PERMEABILITY [10 to 40 inches]
```{r permeability_10to40, warning=FALSE}
f1040$permeability <- findInterval(f1040$ksat_l, permclasses)
f1040 <- mutate_profile(f1040, min_permeability = as.integer(min(permeability, na.rm = TRUE)))
f1040$min_permeability <- names(permclasses)[f1040$min_permeability]
f1040.permeability <- table(f1040$min_permeability, useNA = "ifany")
f1040.permeability.acres <- aggregate(f1040$compacres, by = list(min_permeability = f1040$min_permeability), function(x) round(sum(x, na.rm = TRUE)))
f1040.permeability.acres$n <- f1040.permeability[f1040.permeability.acres$min_permeability]
knitr::kable(f1040.permeability.acres[order(f1040.permeability.acres$x, decreasing = TRUE),],
             col.names = c("Min. Permeability [10 to 40 inches]", "Component Acres", "# Components"),
             row.names = FALSE)
```
...
f0060 <- mutate_profile(f0060, 
                        subsurface_frags_lt3_l = weighted.mean(gravel_l, hzdepb_r - hzdept_r, na.rm = TRUE),
                        subsurface_frags_lt3_r = weighted.mean(gravel_r, hzdepb_r - hzdept_r, na.rm = TRUE),
                        subsurface_frags_lt3_h = weighted.mean(gravel_h, hzdepb_r - hzdept_r, na.rm = TRUE))
...
```{r, results='asis'}
f0060 <- mutate_profile(f0060, 
                        subsurface_frags_gt3_l = weighted.mean(total_frags_pct_l - gravel_l, hzdepb_r - hzdept_r, na.rm = TRUE),
                        subsurface_frags_gt3_r = weighted.mean(total_frags_pct_r - gravel_r, hzdepb_r - hzdept_r, na.rm = TRUE),
                        subsurface_frags_gt3_h = weighted.mean(total_frags_pct_h - gravel_h, hzdepb_r - hzdept_r, na.rm = TRUE))
