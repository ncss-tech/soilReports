...
Shiny Pedon Summary - Static Report
==================
<br>
`r format(Sys.time(), "%Y-%m-%d")`
<br>
<center>`r inputreportname`
...
<p>
```{r hillslope-position-tables, results='asis'}
...
### Geomorphons Summary
<p>
```{r geomorphons-position-tables, results='asis'}
...
<p>
```{r drainage-class-tables, results='asis'}
...
<p>
```{r surface-shape-tables, results='asis'}
...
```{r fig.width=13, fig.height=7, results='asis'}
...
```{r generalize-hz-names-3,fig.width=7, fig.height=7, out.width="90%", results='asis'}
this.data <- t(s$ct)
...
```{r, echo=FALSE, fig.height=4, fig.width=6, results='asis'}
# clay box-whisker plot, grouped by genhz, over-printed with original hz names
# subset data
h.i <- horizons(f)
h.i.sub <- subset(h.i, subset=!is.na(clay), drop=FALSE)
...
<p>
```{r ML-hz-figures, fig.height=5, fig.width=6, results='asis'}
...
### Slice-Wise Plot
```{r, fig.height=6, fig.width=4, results='asis'}
...
```{r profile-matrix-fig-size-reset, results='asis'}
...
<p>
```{r profile-plot, out.width='100%', results='asis'}
par(mar=c(1,1,1,1))
...
<p>
```{r summary-tables-1, results='asis'}
this.data <- summarize.component(f)$tt
this.align <- c('l', rep('c', times=ncol(this.data)))
print(xtable(this.data, align=this.align), type='html', table.placement="H", caption.placement="top", html.table.attributes='cellpadding="1" cellspacing="3"')
```
</p>
...
<p>
```{r summary-tables-2, results='asis'}
this.data <- summarize.component(f)$rt
this.align <- c('l', rep('c', times=ncol(this.data)))
print(xtable(this.data, align=this.align), type='html', table.placement="H", caption.placement="top", html.table.attributes='cellpadding="1" cellspacing="3"')
```
</p>
...
```{r agg-color-plot-d, fig.width=12, fig.height=8, results='asis'}
par(mar=c(4.5, 2, 0, 0))
...
```{r agg-color-plot-m, fig.width=12, fig.height=8, results='asis'}
par(mar=c(4.5, 2, 0, 0))
...
```{r summary-bwplot, fig.height=10, fig.width=15, out.width="100%", results='asis'}
summarize.component(f)$pmg
```
...
```{r summary-tables-3, results='asis'}
this.data <- summarize.component(f)$sf
this.align <- c('l', rep('c', times=ncol(this.data)))
print(xtable(this.data, align=this.align), type='html', table.placement="H", caption.placement="top", html.table.attributes='cellpadding="1" cellspacing="3"')
```
...
```{r summary-tables-4, results='asis'}
this.data <- summarize.component(f)$dt
this.align <- c('l', rep('c', times=ncol(this.data)))
print(xtable(this.data, align=this.align), type='html', table.placement="H", caption.placement="top", html.table.attributes='cellpadding="1" cellspacing="3"')
```

```{r diagnostic-prop-plot, fig.height=12, fig.width=10, results='asis'}
if(nrow(diagnostic_hz(f)))
...
```{r pedon-gis-tables, results='asis'}
this.data <- summarize.component(f)$pg
this.align <- c('l', rep('c', times=ncol(this.data)))
print(xtable(this.data, align=this.align), type='html', table.placement="H", caption.placement="top", html.table.attributes='cellpadding="1" cellspacing="3"')
```

```{r aspect-circular-hist-fig-size-reset, results='asis'}
# this resets the default image width according to the number of profiles
opts_chunk$set(fig.width=4.5)
```

```{r aspect-circular-hist, fig.height=5, results='asis'}
par(mar=c(0,0,0,0))
