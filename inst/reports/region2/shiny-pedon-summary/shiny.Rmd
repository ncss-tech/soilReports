...
=======
>>>>>>> added stable version of Shiny interactive pedon summary / component report; still does not conform to default soilReportsetup (i.e. needs both shiny.Rmd and report.Rmd); may be worth creating a separate tier for Shiny-interfaces that create static reports?
...
=======
    textInput("s.mu",label="Enter pattern for matching MUSYM",value="6054") 
>>>>>>> added stable version of Shiny interactive pedon summary / component report; still does not conform to default soilReportsetup (i.e. needs both shiny.Rmd and report.Rmd); may be worth creating a separate tier for Shiny-interfaces that create static reports?
)
...
inputPanel(
  selectInput("thematic_field", label = "Select horizon attribute data to plot: ",
              choices = c("clay","sand","phfield","total_frags_pct_cal","moist_soil_color","dry_soil_color"),selected = "clay")
)
...
<<<<<<< HEAD
<<<<<<< HEAD
    mapview(pedon_locations)@map #  here we just access the leaflet map slot directly for shiny rendering and use renderLeaflet
=======
    mapview(pedon_locations)@map #there is also a renderMapview() function. ##,popup=popupImage(src = "local",paste0("./graphs/",1:nrow(site(peds)),".png"))
                                 #  here we just access the leaflet map slot directly for shiny rendering and use renderLeaflet
>>>>>>> fixed minor issue with subsetting and removed code to add soilprofile plots to mapview (this is a work in progress). added report metadata to shiny.Rmd (placeholder until issues with metadata and setup are resolved).
=======
    
    
      for(p in 1:nrow(site(peds))) {
        png(paste0("./graphs/",p,".png"),width = 225, height = 350)
        par(mar=c(0,1,0,0))
        plotSPC(peds[p,],width=0.8, cex.names = 1.25,name="hzname",label="taxonname",alt.label = "site_id", alt.label.col = "WHITE",axis.line.offset = -4.5)
        dev.off()
      }
    
    ## popupImage(paste0(Sys.getenv("temp"),"\\",1:nrow(site(peds)),".png"), width = 125, height = 250)
    
    # p <- lapply(1:nrow(site(peds)), function(i) {
    #   plotSPC(peds[i,],width=0.8, cex.names = 1.25,name="hzname",label="taxonname",alt.label = "site_id", alt.label.col = "WHITE",axis.line.offset = -4.5)
    #   recordPlot()
    # })

    mapview(pedon_locations,popup=popupImage(src = "local",paste0("./graphs/",1:nrow(site(peds)),".png")))@map #there is also a renderMapview() function. 
                                 #  here we just access the leaflet map slot directly for shiny rendering and use renderLeaflet
>>>>>>> added stable version of Shiny interactive pedon summary / component report; still does not conform to default soilReportsetup (i.e. needs both shiny.Rmd and report.Rmd); may be worth creating a separate tier for Shiny-interfaces that create static reports?
})
```
