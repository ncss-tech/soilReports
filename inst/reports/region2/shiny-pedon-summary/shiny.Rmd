      ...
      if(!any(file.exists(c("pedon_cache.Rda"))) | !cache_data) { 
        ...
        if(!inherits(pedons_raw, 'try-error')) {
          ...
          if(!inherits(mu, 'try-error')) {
            ...
            if(all(is.na(musymz)))
              musymz <- rep("<missing>", length(pedons))
        ...
        if(cache_data) {
         ...
         if(cache_data) { 
           ...
           file.rename(from="component_cache.Rda",
                       to="component_cache.Rda.bak")
           file.rename(from="spatial_cache.Rda",
                       to="spatial_cache.Rda.bak")
         }
      }
    loaded <<- TRUE
  }
...
if(loaded) {
  ...
  observeEvent(input$export_report, {
    ...
    res <- loadReportData()
    my.env$input <- input
    my.env$pedons <- res$pedons
    
    # render in constructed environment
    render(input='export.Rmd', envir = my.env,
