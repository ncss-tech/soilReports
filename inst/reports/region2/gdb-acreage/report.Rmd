...
get_NASIS_legendmu <- function() {
  soilDB::dbQueryNASIS(soilDB::NASIS(), "SELECT * FROM lmapunit_View_1 lmv;")
}
...
legend_mu2 <- legend_mu1[,c("areasymbol", "muiidref", "musym",
                            "mustatus", "muacres")]
legend_mu3 <-  rbind(subset(legend_mu2, areasymbol == eval(area.symbol)))
names(legend_mu3) <- c("Area", "MURecID", "MUSYM",
                       "Status", "Legend_Acres")
