...
## TODO: figure out how to parameterize with _effective_ sampling size
##       computed via Moran's I
...
custom.bwplot <- function(x, coef=NA, do.out=FALSE) {
  ...
  n <- length(na.omit(x))
  # this requires spatial data to compute...
  # n_eff <- effective_n()
...
```{r, echo=FALSE, eval=correct.sample.size}
spatial.stats.summary <- ddply(sampling.res$spatial.stats, c('.id', 'variable'), summarize, I=median(Moran_I, na.rm = TRUE), n_eff=sum(n_eff, na.rm = TRUE))

# kable(dcast(spatial.stats.summary, variable ~ .id, value.var = 'I'), row.names = FALSE, caption = "Median Moran's I")
kable(spatial.stats.summary, row.names = FALSE, caption = "Spatial Statistics")
```
