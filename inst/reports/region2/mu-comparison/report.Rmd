...
output:
  html_document:
    ...
    keep_md: no
---
<!-- mu-comparison (v3.3.2) -- instance created 2017-12-29 10:41:30-->  
...
# Last two (aqp and sharpshootR) are (in general) installed from GitHub. Maybe in the future (when more stable) we will rely more heavily on CRAN.
# TODO: Check installed package against latest remote commit (requires devtools) to provide warning if there is a newer version?
# TODO: should the report install CRAN packages if it discovers they are missing? or should this only be done via reportSetup?
#      and what about dependencies? and GitHub packages? report doesn't know how to distinguish package source, but setup does.
#      currently, scheme is to load packs if you have 'em and fails if you don't. If it fails, tells you to run reportSetup.
#if(length(newpackz)) 
...
if(any(!accessible.inputs)) {
  stop(paste0("The following input files either do not exist or are unreadable: ",paste0(unlist(raster.list[!accessible.inputs]), collapse=", ")))
}
### -----
...
if(any(grepl(basename(outputfiles), pattern='([/\\|<>:\\*?\"])'))) {
  stop("Map unit set or output file name contains invalid characters for filename. Either override default output filenames or remove  [/\\|<>:\\*?\"] characters from your map unit symbols.", call. = FALSE)
}
### -----
...
if(nrow(polygons.to.check) > 0) {
  # BUG: this fails when mu.set contains illegal characters
  # https://github.com/ncss-tech/soilReports/issues/75
