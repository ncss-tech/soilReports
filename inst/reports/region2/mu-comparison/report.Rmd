...
if(nrow(polygons.to.check) > 0) {
  ...
  try(writeOGR(mu.check, dsn='output', layer=shp.fname, driver='ESRI Shapefile', overwrite_layer=TRUE))
  try(write.csv(mu.check,file=paste0("output/", shp.fname, ".csv")))
}
...
if(length(sampling.res$unsampled.ids) > 0) {
  ...
  # BUG: this fails when mu.set contains illegal characters
  # https://github.com/ncss-tech/soilReports/issues/75
  try(writeOGR(mu[sampling.res$unsampled.ids, ], dsn='output', layer=shp.fname, driver='ESRI Shapefile', overwrite_layer=TRUE))
}
...
# BUG: this fails when mu.set contains illegal characters
# https://github.com/ncss-tech/soilReports/issues/75
try(write.csv(poly.stats.wide, file=poly.stats.fname, row.names=FALSE))
...
# BUG: this fails when mu.set contains illegal characters
# https://github.com/ncss-tech/soilReports/issues/75
try(writeOGR(mu, dsn='output', layer=shp.fname, driver='ESRI Shapefile', overwrite_layer=TRUE))
