...
if(file.exists('cached-samples.Rda')) {
  ...
  # result is a list
  sampling.res <- sampleRasterStackByMU(mu, mu.set, mu.col, raster.list, pts.per.acre)
...
### Raster Data Sources
```{r, echo=FALSE}
kable(sampling.res$raster.summary, row.names = FALSE)
```
...
bwplot(.id ~ value | variable, data=sampling.res$raster.samples, stats=custom.bwplot, scales=list(y=list(alternating=3), x=list(relation='free', tick.number=10)), as.table=TRUE, col='black', layout=c(1, length(unique(sampling.res$raster.samples$variable))), strip=strip.custom(bg=grey(0.85)), xlab='', par.settings=tps, 
panel=function(...) {
...
## issues: 
# 1. possible methods: https://github.com/ncss-tech/soilReports/issues/18
# 2. add geomorphons, curvature classses, and possibly NLCD (factors)
...
## TODO: perform checking by .id first, filter out low-varance MU
...
if(any(unlist(sd.by.id[, -1]) < 1e-5)) {
  ...
  ###
  ### TODO: vegan functions may be more useful here
  dd <- vegan::betadisper(d.dist, group=d.sub$.id, bias.adjust = TRUE, sqrt.dist = TRUE, type='median')
  vegan:::plot.betadisper(dd, ellipse = TRUE, hull = FALSE, col=cols, conf=0.5, segments=FALSE)
...
```{r, echo=FALSE, fig.width=10, fig.height=8, eval=MDS.summary}
par(mar=c(2,5,2,2))
## note that we don't load the Hmisc package as it causes many NAMESPACE conflicts
plot(Hmisc::varclus(as.matrix(d.sub[, -c(1:3)])))
```


### Raster Data Importance

```{r, echo=FALSE, fig.width=8, fig.height=6, eval=MDS.summary}
...
m <- randomForest(x=d.sub[, -c(1:2)], y=factor(d.sub$.id), importance = TRUE)
