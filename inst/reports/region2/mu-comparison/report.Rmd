...
output:
  html_document:
    ...
    keep_md: no
---
<!-- mu-comparison (v3.0) -- instance created 2017-08-07 12:11:58-->  
...
makeCategoricalOutput <- function(dat, do.spatial.summary=T) {
  ...
  # re-set MUSYM factor levels, have to do this because sweepProportions doesn't retain these levels
  ...
  if(length(unique(x.long$.id)) > 1 & do.spatial.summary) {
    ...
    trellis.par.set(tps)
    cat('  \n  \n')
    print(barchart(.id ~ value, groups=x.long$label, data=x.long, horiz=TRUE, stack=TRUE, xlab='Proportion of Samples', scales=list(cex=1.5), key=simpleKey(space='top', columns=3, text=levels(x.long$label), rectangles = TRUE, points=FALSE)))
    cat('  \n  \n')
  }  
