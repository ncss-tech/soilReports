...
if(exists('mu.set')) { #TODO: this type of if(exists(obj)) needs to wrap all references to report parameters 
  ...
  if(any(mu_nosp)) #check if any of the predefined musyms are absent from the spatial
    mu.set <- mu.set[-which(mu_nosp)] #if so, remove them
  if(length(mu.set) == 0) #if we removed everything (e.g. no musyms match due to wrong config file?), fail gracefully
  ...
  mu.set <- sort(unique(mu[[mu.col]])) #if mu.set is not predefined, ordering is determined by the order of musyms in the source file
}
