...
packz <- c("MASS","rgdal","rgeos","raster","plyr",
           "latticeExtra","cluster","clhs","randomForest",
           "spdep","reshape2","aqp","sharpshootR","RColorBrewer")
...
l_ply(split(mu.stats, mu.stats$variable), function(i) {
  ...
  print(kable(i, caption = var.name, row.names = FALSE, 
              align = 'r', digits = dg, col.names = c(mu.col, names(i)[-1])))
})
...
res <- ldply(d.circ.list, function(i) {
  mu <- unique(i$.id)
  circ.stats <- aspect.plot(i$value, q = c(0.1, 0.5, 0.9), 
                            plot.title = mu, pch = NA, bg = 'RoyalBlue', 
                            col = 'black', arrow.col = c('grey', 'red', 'grey'), 
                            stack = FALSE, p.bw = 90)
...
if (nrow(polygons.to.check) > 0) {
  try(writeOGR(mu.check, dsn = 'output', layer = shp.qc.fname, 
...
if (length(sampling.res$unsampled.ids) > 0) {
  try(writeOGR(mu[sampling.res$unsampled.ids, ], dsn = 'output', 
               layer = shp.unsampled.fname, 
               driver = 'ESRI Shapefile', overwrite_layer = TRUE))
}
...
# save to file
try(writeOGR(mu, dsn = 'output', layer = shp.stats.fname, 
             driver = 'ESRI Shapefile', overwrite_layer = TRUE))
...
```{r, echo=FALSE, results='asis'}
kable(head(slot(mu.check[complete.cases(slot(mu.check, 'data')),], 'data')), row.names = FALSE)
```
